...
import (
	...
	"github.com/darkphotonKN/fireplace/services/orchestrator-service/config"
...
var (
	...
	// gRPC port — ordinal 5 in the 710X scheme (auth 7101, example 7102,
	// plan 7103, calendar 7104, orchestrator 7105). Read from the env directly;
	// the fallback only applies when GRPC_ORCHESTRATOR_ADDR is unset.
	grpcAddr       = commonhelpers.GetEnvString("GRPC_ORCHESTRATOR_ADDR", "7105")
...
func main() {
	...
	broker.DeclareExchange(ch, commonconstants.OrchestratorEventsExchange, "topic")
	defer func() {
		closeCh()
		ch.Close()
	}()
	...
	go func() {
		slog.Info("orchestrator-service gRPC server starting", "port", grpcAddr)
